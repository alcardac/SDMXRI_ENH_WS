...
namespace Estat.Sri.MappingStoreRetrieval.Constants
{
    ...
    {
        /// <summary>
        /// Gets the SQL Query format/template for retrieving the Attribute attachment groups from a DSD id. Use with <see cref="string.Format(string,object)"/> and one parameter the <see cref="ParameterNameConstants.IdParameter"/>
        /// </summary>
        public const string AttributeAttachmentGroupQueryFormat =
            "SELECT GR.ID, COMP.ID AS ATTR_REF FROM COMPONENT COMP INNER JOIN ATT_GROUP AG ON COMP.COMP_ID = AG.COMP_ID INNER JOIN DSD_GROUP GR ON AG.GR_ID = GR.GR_ID WHERE COMP.DSD_ID = {0} ";


        /// <summary>
        /// Gets the SQL Query format/template for retrieving the Attribute attachment measures from a DSD id. Use with <see cref="string.Format(string,object)"/> and one parameter the <see cref="ParameterNameConstants.IdParameter"/>
        /// </summary>
        public const string AttributeAttachmentMeasureQueryFormat =
            "SELECT XSM.ID as XSM_ID, ATTR.ID AS ATTR_ID FROM COMPONENT ATTR INNER JOIN ATT_MEASURE AM ON AM.ATT_COMP_ID = ATTR.COMP_ID INNER JOIN COMPONENT XSM ON XSM.COMP_ID = AM.MEASURE_COMP_ID WHERE ATTR.DSD_ID = {0}";
