    ...
    internal class DsdRetrievalEngine : ArtefactRetrieverEngine<IDataStructureMutableObject>
    {
            ...
            : base(mappingStoreDb)
        {
            ...
            this._componentAnnotationRetrieverEngine = new IdentifiableAnnotationRetrieverEngine(
                mappingStoreDb,
                new ItemTableInfo(SdmxStructureEnumType.Component) { ForeignKey = "DSD_ID", PrimaryKey = "COMP_ID", Table = "COMPONENT" });

            this._groupAnnotationRetrieverEngine = new IdentifiableAnnotationRetrieverEngine(
             mappingStoreDb,
             new ItemTableInfo(SdmxStructureEnumType.Group) { ForeignKey = "DSD_ID", PrimaryKey = "GR_ID", Table = "DSD_GROUP" });
        }
        ...
        private void SetupGroupAttributes(IDataStructureMutableObject dataStructure, long dsdId)
        {
            ...
            var idToAttribureMap = dataStructure.Attributes.Where(o => o.AttachmentLevel == AttributeAttachmentLevel.Group).ToDictionary(o => o.Id, StringComparer.Ordinal);
            if (idToAttribureMap.Count == 0)
            {
                return;
            }

            using (DbCommand command = this._commandBuilder.Build(new ItemSqlQuery(this._attributeGroupQueryInfo, dsdId)))
            {
                using (IDataReader dataReader = this.MappingStoreDb.ExecuteReader(command))
                {
                    while (dataReader.Read())
                    {
                        ...
                        {
                            _log.WarnFormat("Attribute with ID {0} of DSD {1} has group mappings but its attachment level is not group", attributeId, dataStructure.Id);                           
                        }
                    }
                }
            }
